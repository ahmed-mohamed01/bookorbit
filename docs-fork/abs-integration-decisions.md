...
| 2   | **One-way, ABS -> BookOrbit** (see the concurrency note below)               | Sync direction stays one-way: ABS is the source of truth for position, BookOrbit never pushes back. What no longer holds is the old rationale that there is no local writer to race.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
**Concurrency with the web player (supersedes the old "no local writer to race" claim).** Since
upstream v2.10.0 the audiobook player is a first-class writer on the same `audiobook_progress` row:
`AudiobookRepository.updatePlaybackState` is a compare-and-swap on the new `revision` column, and the
row also carries `captured_at`, `operation_id` and `manifest_revision`. So ABS and the player share
one row with real optimistic concurrency, and ABS must speak that contract rather than a private one.
`upsertAudioProgressGuarded` therefore predicates on `revision` (not `updated_at`), bumps it on every
applied write, and stamps `captured_at` with the ABS-side `lastUpdate` of the position so the API
reports when the listening happened rather than when the poll landed. `operation_id` and
`manifest_revision` identify a player save and stay untouched. The practical effect is symmetric: a
player save that lands between the sync pre-load and the apply makes the ABS CAS miss and the write is
deferred, and an ABS write that lands under an open player session bumps the revision the player holds,
so its next save gets a 409 and it reloads instead of silently overwriting.
...
- `client/src/router/index.ts` - drop the `settings-audiobookshelf` route + `INTEGRATION_ROUTES`
  entry (upstream deleted `IntegrationAllSettings.vue` in its per-route settings restructure;
  same removal cost as any sibling integration route)
- `client/src/features/settings/lib/settings-nav.ts` - drop the nav item
