...
describe('useAudiobookshelfSettings', () => {
  ...
  it('saveSettings reloads libraries when credentials change and the token is configured', async () => {
    ...
    mockFetchLibraries.mockResolvedValue({ libraries: [{ id: 'lib-1', name: 'Audiobooks', mediaType: 'book', provider: 'abs', excluded: false }] })
    const c = await loadComposable()

    await c.saveSettings({ apiToken: 'new-token' })

    expect(mockFetchLibraries).toHaveBeenCalledTimes(1)
    expect(c.libraries.value).toEqual([{ id: 'lib-1', name: 'Audiobooks', mediaType: 'book', provider: 'abs', excluded: false }])
  })
  ...
  it('fetchLibraries keeps only the latest response when calls overlap', async () => {
    ...
    resolvers[1]!({ libraries: [{ id: 'fresh', name: 'Fresh', mediaType: 'book', provider: 'abs', excluded: false }] })
    resolvers[0]!({ libraries: [{ id: 'stale', name: 'Stale', mediaType: 'book', provider: 'abs', excluded: false }] })
    await Promise.all([first, second])

    expect(c.libraries.value).toEqual([{ id: 'fresh', name: 'Fresh', mediaType: 'book', provider: 'abs', excluded: false }])
  })
})
