...
export function haveEqualTokenSet(a: string[], b: string[]): boolean {
...
export function scoreAuthor(a: string, b: string): number {
...
export function scoreAuthors(candidateAuthors: string[], localAuthors: string[]): number {
  let best = 0;
  for (const candidateAuthor of candidateAuthors) {
    for (const localAuthor of localAuthors) {
      best = Math.max(best, scoreAuthor(candidateAuthor, localAuthor));
    }
  }
  return best;
}
