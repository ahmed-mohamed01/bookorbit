...
function parseSeriesEntry(entry: string): { seriesName: string; seriesIndex: string | null } {
  const match = entry.match(/ #([^#\s]+)$/);
  // parseSeriesIndex is the gate every other extractor uses; the DB check constraint rejects
  // anything else (signed, trailing dot, bare fraction), so an unparseable suffix stays in the name.
  const seriesIndex = match ? parseSeriesIndex(match[1]) : null;
  if (match && seriesIndex != null) {
    const name = entry.slice(0, match.index).trim();
    return { seriesName: name.length > 0 ? name : entry, seriesIndex };
  }
  return { seriesName: entry, seriesIndex: null };
}

function mapSeriesMemberships(value: unknown): { seriesName: string; seriesIndex: string | null }[] {
  const memberships: { seriesName: string; seriesIndex: string | null }[] = [];
  const seen = new Set<string>();
...
export function mapAbsMetadata(raw: AbsBookMetadata): ParsedBookData {
  ...
  return {
    ...
    // Absent means the sidecar does not speak to this and must not erase audio-derived values;
    // a declared-but-empty list is an explicit clear and stays [].
    narrators: source.narrators === undefined ? undefined : coerceStringArray(source.narrators),
    chapters: source.chapters === undefined ? undefined : mapChapters(source.chapters),
