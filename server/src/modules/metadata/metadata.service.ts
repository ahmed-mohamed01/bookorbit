...
export class MetadataService {
  ...
  /**
   * Publishes the repaired thumbnail from a fully written temporary file, so an interrupted write
   * can never leave a truncated thumbnail at the served path. Nothing validates a thumbnail beyond
   * its filename, so a truncated one would be permanent.
   *
   * A hard link is preferred because it also fails when the path already exists, leaving a cover
   * write that landed first in possession of its own thumbnail. Network mounts without hard links
   * fall back to rename, which is still atomic and so still cannot publish a partial file; it only
   * loses the exclusive create, and losing that race costs a thumbnail that is stale by
   * milliseconds rather than one that is broken forever.
   */
  private async publishRepairedThumbnail(dir: string, thumbnailPath: string, bytes: Buffer): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        // No thumbnail published yet, so this repair still has something to contribute.
      }
