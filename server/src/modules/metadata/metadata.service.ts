...
export class MetadataService {
  ...
  async applyCoverSource(bookId: number, source: CoverSource): Promise<CoverSourceApplyOutcome> {
    ...
    try {
      ...
      // Stored as a sidecar image ('opf' origin), not a folder image: the slot reconciler re-resolves
      // folder-image slots from embedded art whenever files change, which would undo sidecar precedence.
      const medium = await this.coverStore.chooseSidecarMedium(bookId, bytes);
      if (await this.bookMetadataLockService.isFieldLocked(bookId, medium === 'ebook' ? 'cover' : 'audioCover')) {
        this.logger.debug(
      ...
      await this.persistCover(bookId, medium, bytes, false, 'opf');
      this.logger.debug(
        `[${event}] [end] bookId=${bookId} kind="${kind}" medium=${medium} durationMs=${Date.now() - startedAt} applied=true - cover source application completed`,
      );
      return 'saved';
    } catch (error) {
      const errorClass = error instanceof Error ? error.name : 'Error';
