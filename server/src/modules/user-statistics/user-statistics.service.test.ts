...
describe('UserStatisticsService', () => {
    ...
    expect(result).toEqual([
      {
        ...
        eventsCount: 2,
        bySource: { bookorbit: 90, koreader: 0, kobo: 30, audiobookshelf: 0 },
      },
      { day: '2026-04-07', readingSeconds: 0, progressDelta: 0, eventsCount: 0, bySource: { bookorbit: 0, koreader: 0, kobo: 0, audiobookshelf: 0 } },
      { day: '2026-04-08', readingSeconds: 0, progressDelta: 0, eventsCount: 0, bySource: { bookorbit: 0, koreader: 0, kobo: 0, audiobookshelf: 0 } },
    ]);
  });
  ...
  it('returns all 24 hour buckets for peak reading hours', async () => {
    ...
    expect(result[8]).toEqual(
      expect.objectContaining({
        ...
        readingSeconds: 600,
        eventsCount: 3,
        bySource: { bookorbit: 400, koreader: 0, kobo: 200, audiobookshelf: 0 },
      }),
    );
    expect(result[21]).toEqual(
      expect.objectContaining({
        ...
        readingSeconds: 900,
        eventsCount: 4,
        bySource: { bookorbit: 0, koreader: 900, kobo: 0, audiobookshelf: 0 },
      }),
    );
    expect(result[0]).toEqual(
      expect.objectContaining({ hour: 0, readingSeconds: 0, eventsCount: 0, bySource: { bookorbit: 0, koreader: 0, kobo: 0, audiobookshelf: 0 } }),
    );
  });
  ...
  it('returns all weekday buckets for favorite reading days', async () => {
    ...
    expect(result[1]).toEqual({
      ...
      bySource: { bookorbit: 600, koreader: 1200, kobo: 0, audiobookshelf: 0 },
    });
    expect(result[0]).toEqual({
      dayOfWeek: 0,
      readingSeconds: 0,
      eventsCount: 0,
      byFormat: {},
      bySource: { bookorbit: 0, koreader: 0, kobo: 0, audiobookshelf: 0 },
    });
  });
  ...
  it('delegates and caches archetype/genre/pace/chord queries', async () => {
    ...
    await expect(service.getGenreReadingTime(user, { libraryIds: [1] })).resolves.toEqual([
      { genre: 'Sci-Fi', readingSeconds: 300, bySource: { bookorbit: 200, koreader: 0, kobo: 100, audiobookshelf: 0 } },
    ]);
