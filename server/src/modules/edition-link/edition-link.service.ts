...
import type { EditionLinkCandidate, EditionLinkCounterpartSummary } from '@bookorbit/types';
...
export class EditionLinkService {
  ...
  async getForBook(
    user: RequestUser,
    bookId: number,
  ): Promise<{ link: BookEditionLink | null; proposed: EditionLinkCandidate | null; counterpart: EditionLinkCounterpartSummary | null }> {
    ...
    if (link) {
      ...
      if (!(await this.canAccess(counterpartId, user))) return { link: null, proposed: null, counterpart: null };
      const counterpart = await this.repo.findBookSummary(counterpartId);
      return { link, proposed: null, counterpart };
    }

    const modality = await this.repo.getBookModality(bookId);
    if (modality !== 'text' && modality !== 'audio') {
      return { link: null, proposed: null, counterpart: null };
    }

    const [proposed] = await this.findCandidates(user, bookId, modality);
    return { link: null, proposed: proposed ?? null, counterpart: null };
  }
