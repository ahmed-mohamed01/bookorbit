...
describe('EditionLinkController', () => {
  ...
  it('delegates all HTTP operations to the service', async () => {
    const link = { id: 1 };
    service.getForBook.mockResolvedValue({ link: null, proposed: null, counterpart: null });
  ...
  it('serializes the DB row to the shared wire contract (Date -> ISO string, nullable createdBy)', async () => {
    ...
    service.getForBook.mockResolvedValue({ link: row, proposed: null, counterpart });
    ...
    expect(await controller.getForBook(user, 10)).toEqual({ link: expected, proposed: null, counterpart });
  });
});
