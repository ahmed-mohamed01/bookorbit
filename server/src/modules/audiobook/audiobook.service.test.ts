...
function makeFixture() {
  ...
  return {
    ...
    bookService,
    achievementEvents,
    service: new AudiobookService(repo as never, bookService as never, achievementEvents as never),
  };
}

function makePlaybackWrite(manifestRevision: string, assetId: string) {
  return {
    assetId,
    positionMs: 5_000,
...
describe('AudiobookService', () => {
  ...
  it('emits book.progress-changed (web_reader) when the persisted percentage changed', async () => {
    const { service, repo, achievementEvents } = makeFixture();
    const manifest = await service.getManifest(30, makeUser());
    repo.findPlaybackState.mockResolvedValue({ percentage: 20, operationId: '88888888-8888-4888-8888-888888888888' });
    repo.updatePlaybackState.mockResolvedValue({ capturedAt: new Date('2026-02-01T00:00:00.000Z'), revision: 2 });

    const saved = await service.putPlaybackState(30, makePlaybackWrite(manifest.revision, manifest.assets[1]!.assetId), makeUser());
  ...
  it('does not emit book.progress-changed when the audio percentage is unchanged', async () => {
    const { service, repo, achievementEvents } = makeFixture();
    const manifest = await service.getManifest(30, makeUser());
    const percentage = (15_125 / 30_125) * 100;
    repo.findPlaybackState.mockResolvedValue({ percentage, operationId: '88888888-8888-4888-8888-888888888888' });
    repo.updatePlaybackState.mockResolvedValue({ capturedAt: new Date('2026-02-01T00:00:00.000Z'), revision: 2 });

    await service.putPlaybackState(30, makePlaybackWrite(manifest.revision, manifest.assets[1]!.assetId), makeUser());
