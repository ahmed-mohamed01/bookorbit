...
CREATE TABLE IF NOT EXISTS "storyteller_read_along_builds" (
	...
	CONSTRAINT "storyteller_read_along_builds_status_chk" CHECK ("status" in ('building', 'ready', 'failed')),
...
DO $$ BEGIN
	IF NOT EXISTS (
		...
		ALTER TABLE "storyteller_read_along_builds" ADD CONSTRAINT "storyteller_read_along_builds_status_chk" CHECK ("status" in ('building', 'ready', 'failed'));
	END IF;
END $$;
--> statement-breakpoint
DO $$ BEGIN
