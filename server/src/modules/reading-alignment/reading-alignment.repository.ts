...
export type ReadingProgressSnapshot = { percentage: number; updatedAt: Date };
export type AudiobookProgressSnapshot = { currentFileId: number; positionSeconds: number; percentage: number; updatedAt: Date };
...
export type AudioFileWithPath = { fileId: number; absolutePath: string; durationSeconds: number | null };

// The book's primary EPUB content file, used both to feed the spine extractor and to hash for change
// detection.
export type EbookFileRef = { id: number; absolutePath: string; sizeBytes: number | null };

export type AlignmentProgress = { samplesDone: number; anchorCount: number };
