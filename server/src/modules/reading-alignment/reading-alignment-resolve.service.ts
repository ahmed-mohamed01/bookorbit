...
// Only 'ebook' is resolved: opening the ebook resumes it from newer audiobook progress. The reverse
// (resuming the audio player from newer ebook progress) is handled eagerly by the progress sync
// (projectEbookToAudio), so there is no 'audio' target/consumer here.
export type CrossFormatResumeTarget = 'ebook';
...
export class ReadingAlignmentResolveService {
  ...
  // Resolve where to resume in the format the user is OPENING (`target`) when the OTHER format's
  // progress is strictly newer. Read-only: no progress is written and no projection is persisted
  // beyond backfilling missing anchor fractions.
  async resolveResume(bookId: number, user: RequestUser, target: CrossFormatResumeTarget): Promise<CrossFormatResume> {
    this.logger.log(`[${RESOLVE_EVENT}] [start] bookId=${bookId} userId=${user.id} target=${target} - cross-format resume started`);

    await this.assertAccess(bookId, user);
    const pair = await this.pairService.resolveAlignmentPair(bookId);
    if (!pair) {
      return this.done(bookId, null, target, UNAVAILABLE);
    }
    ...
    if (!ready || ready.anchors.length < MIN_ANCHORS) {
      return this.done(bookId, pair, target, UNAVAILABLE);
    }
    ...
    return this.done(bookId, pair, target, result);
  }
  ...
  private async resolveAnchors(bookId: number, ebookFileId: number | null, user: RequestUser, rows: AudiobookAlignmentAnchor[]): Promise<Anchor[]> {
    ...
    if (!needsBackfill) {
      await this.assertAccess(bookId, user);
      return rows.filter((row) => row.ebookFraction != null).map(toAnchor);
    }
  ...
  private done(bookId: number, pair: ReadingAlignmentPair | null, target: CrossFormatResumeTarget, result: CrossFormatResume): CrossFormatResume {
    const pairFields = pair ? ` textBookId=${pair.textBookId} audioBookId=${pair.audioBookId}` : '';
    this.logger.log(
      `[${RESOLVE_EVENT}] [end] bookId=${bookId}${pairFields} target=${target} available=${result.available} - cross-format resume completed`,
    );
    return result;
  }
}

// Callers filter out null-fraction rows before mapping; the `?? 0` only satisfies the type and is
// never reached in practice.
function toAnchor(row: AudiobookAlignmentAnchor): Anchor {
  return { audioSeconds: row.audioSeconds, ebookFraction: row.ebookFraction ?? 0, spineIndex: row.spineIndex, phrase: row.phrase };
}
