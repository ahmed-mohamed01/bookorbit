...
describe('ReadingAlignmentResolveService.resolveResume', () => {
  ...
  it('target=ebook: returns phrase + spineIndex when audio progress is newer', async () => {
    const { service } = build({
    ...
    const result = await service.resolveResume(BOOK_ID, USER, 'ebook');
  ...
  it('target=ebook: unavailable when audio progress is missing', async () => {
    const { service } = build({ ebookProgress: { percentage: 0, updatedAt: OLDER }, audioProgress: undefined });
    expect(await service.resolveResume(BOOK_ID, USER, 'ebook')).toEqual({ available: false });
  });
  ...
  it('is gracefully unavailable when the book has no alignment pair', async () => {
    ...
    await expect(service.resolveResume(BOOK_ID, USER, 'ebook')).resolves.toEqual({ available: false });
  ...
  it('enforces content-filter-aware access on the cached-fraction path', async () => {
    ...
    await service.resolveResume(BOOK_ID, USER, 'ebook');
  ...
  it('propagates a Forbidden from the access check', async () => {
    ...
    await expect(service.resolveResume(BOOK_ID, USER, 'ebook')).rejects.toBeInstanceOf(ForbiddenException);
  });
});
