...
export interface MatchOptions {
  ...
  // Minimum similarity a region must reach to be accepted.
  minConfidence?: number;
  // Approximate target length (in words) of the lifted phrase before uniqueness
  // extension. The phrase may grow beyond this to become unique.
  phraseWords?: number;
}
...
export function matchTranscript(transcript: string, spines: SpineText[], opts?: MatchOptions): MatchResult {
  ...
  const phraseWords = opts?.phraseWords ?? DEFAULT_PHRASE_WORDS;
  ...
  const phrase = liftUniquePhrase(best, phraseWords);
