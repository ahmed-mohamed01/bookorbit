...
export interface MetadataSourceProvider {
  ...
  /**
   * Optional: the source's cover files for a bounded set of books, for the bulk cover-refresh path
   * (which reads persisted files rather than an in-memory scan list). The caller passes the books it
   * is about to refresh, so neither side materializes a whole library. Absent means this source
